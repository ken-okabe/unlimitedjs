...
const main = () => {
    ...
    const listNodeTL = (listTL => T(self => listTL.sync(list => self.now =
        h("ul", null, list.map((item, index) => h("li", null, todoNode(item)(index)(editTL.now)(T(self => self.now = item))))))))(listTL);
    const todoNode = item => index => edit => editTextTL => right(edit.mode === undefined
        ? edit.mode = false
        : undefined)(h("div", null,
        todoItemNode(item)(index)(edit)(editTextTL),
        todoEditNode(index)(edit)(editTextTL),
        todoRemoveNode(index)(edit)));
    const todoItemNode = item => index => edit => editTextTL => (edit.mode) && (edit.index === index)
        ? h("input", { type: "text", id: "edit", value: item, onchange: e => editTextTL.now = e.currentTarget.value })
        : item;
    const todoEditNode = (editTL => index => edit => editTextTL => h("button", { type: "button", disabled: edit.mode &&
        ...
        ? "EditDone"
        : "Edit"))(editTL);
    const todoRemoveNode = (removeTL => index => edit => h("button", { type: "button", disabled: edit.mode, onclick: () => removeTL.now = index }, "Remove"))(removeTL);
